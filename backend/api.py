...
@app.route("/cars/fetch_by_branch", methods=["GET"])
def fetch_by_branch():
    branch = request.args.get("branch")
...
def batch_edit_vehicles():
    ...
    for update in updates_list:
        if "reg" not in update or "updates" not in update:
            return (
                jsonify(
                    {"error": "Each update must contain 'reg' and 'updates' fields"}
                ),
                400,
            )
        ...
        for v in vehicles:
            if v["vrm"] == vehicle_reg:
                v.update(updates)
                results.append({"reg": vehicle_reg, "status": "success", "vehicle": v})
                vehicle_found = True
                break

        if not vehicle_found:
            results.append(
                {"reg": vehicle_reg, "status": "error", "message": "Vehicle not found"}
            )
